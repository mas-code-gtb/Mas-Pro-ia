...
const PurchaseList = () => {
  ...
  const fetchPurchases = async () => {
    try {
      ...
      const response = await axios.get('https://maspro-backend.onrender.com/api/purchases/purchase-orders/', config);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`https://maspro-backend.onrender.com/api/purchases/purchase-orders/${id}/`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
