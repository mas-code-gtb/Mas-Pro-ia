...
const EditAccount = () => {
  ...
  const fetchAccount = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:8000/api/accounting/accounts/${id}/`, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/api/accounting/accounts/${id}/`, accountData, config);
