...
const SalesList = () => {
  ...
  const fetchSales = async () => {
    try {
      ...
      const response = await axios.get('https://maspro-backend.onrender.com/api/sales/salesorders/', config);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      // ⭐ CORRECTION : sales-orders → salesorders (sans tiret)
      await axios.delete(`https://maspro-backend.onrender.com/api/sales/salesorders/${id}/`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
