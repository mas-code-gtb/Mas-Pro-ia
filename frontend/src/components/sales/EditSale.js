...
const EditSale = () => {
  ...
  const fetchSale = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:8000/api/sales/salesorders/${id}/`, config);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ⭐ CORRECTION : sales-orders → salesorders (sans tiret)
      await axios.put(`http://127.0.0.1:8000/api/sales/salesorders/${id}/`, saleData, config);
