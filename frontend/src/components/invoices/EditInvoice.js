...
const EditInvoice = () => {
  ...
  const fetchInvoice = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:8000/api/sales/invoices/${id}/`, config);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/api/sales/invoices/${id}/`, invoiceData, config);
